function MenuOption()
    local menuOption = {
        ...
        init = function(self)
            -- Ładowanie panelu z rodzicem modules.client_options:getPanel()
            self.optionPanel = g_ui.loadUI('menuOption', modules.client_options:getPanel())
            
            -- Zarejestruj panel jako lootStatsPanel w client_options
            if modules.client_options and modules.client_options.panels then
                modules.client_options.panels.lootStatsPanel = self.optionPanel
            end
        ...
        terminate = function(self)
            ...
            -- Nowy system nie wymaga removeTab
        ...
        setDefaultValuesToElementsUI = function(self)
            ...
            local amountValue = store:getAmountLootOnScreen()
            self.elements.amountLootOnScreen:setValue(amountValue)
            self.elements.amountLootOnScreenLabel:setText(tr('Amount of loot on screen: %d', amountValue))
            
            local delayValue = store:getDelayTimeLootOnScreen()
            self.elements.delayTimeLootOnScreen:setValue(delayValue)
            self.elements.delayTimeLootOnScreenLabel:setText(tr('Delay time (ms): %d', delayValue))
        ...
        onChangeUIAmountLootOnScreen = function(self, widget, value)
            self.elements.amountLootOnScreenLabel:setText(tr('Amount of loot on screen: %d', value))
            store:setAmountLootOnScreen(value)
        end;

        onChangeUIDelayTimeLootOnScreen = function(self, widget, value)
            self.elements.delayTimeLootOnScreenLabel:setText(tr('Delay time (ms): %d', value))
