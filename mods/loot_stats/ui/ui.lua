...
-- Global variable for confirmation window
saveOverWindow = nil
...
function UI()
    local ui = {
        ...
        init = function(self)
            ...
            -- Użycie nowego API dla przycisków
            self.moduleButton = modules.client_topmenu.addRightGameToggleButton('lootStatsButton', tr('Loot Stats'), '/mods/loot_stats/ui/img/icon', function() self:toggle() end)
            if self.moduleButton then
            ...
            if self.mainWindow then
                ...
                g_logger.error('[Loot Stats] Failed to load main window')
                return
            end
        ...
        terminate = function(self)
            ...
            -- Clean global confirmation window
        ...
        toggle = function(self)
            if not self.moduleButton or not self.mainWindow then
                return
            end
        ...
        refreshLootItems = function(self, monsterName)
            ...
            for a,b in pairs(itemTable) do
                ...
                -- Wyciągnij czystą nazwę z formatu {ID|name}
                local displayName = a
                local nameMatch = string.match(a, '{%d+|(.+)}')
                if nameMatch then
                ...
                self.listElements[a]:getChildById('text'):setText(text)
                 -- Wyciągnij ID z komunikatu serwera w formacie {ID|name}
                 local itemId = nil
                 
                 -- Sprawdź czy nazwa ma format {ID|name}
                 local idMatch = string.match(a, '{(%d+)|')
                 if idMatch then
                     itemId = tonumber(idMatch)
                 else
                     -- Fallback: szukaj w items.xml jeśli format jest inny
                     if items and type(items) == "table" then
                         for id, item in pairs(items) do
                             if item.name and item.name:lower() == a:lower() then
                                 itemId = id
                                 break
                             end
                         end
                 ...
                 -- W OTClient-Redemption itemId == clientId
                 -- Tworzymy item bezpośrednio używając itemId
                 local clientId = itemId or 3547 -- fallback do default itemu
        ...
        clearData = function(self)
            -- Wyczyść wszystkie dane z store
            store:clear()
            store.lootStatsTable = {}
            
            -- Odśwież UI
            self:destroyListElements()
            self.actualVisibleTab = { tab = 0, info = 0 }
            self:setDefaultValuesToElementsUI()
            
            print("[Loot Stats] All data cleared successfully")
        end;
